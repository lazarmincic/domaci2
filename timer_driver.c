...
static void write_timer(unsigned int dani,unsigned int sati,unsigned int minuti,unsigned int sekunde);
{
	...
	sec = dani* 86400 + sati*3600 + minuti*60 + sekunde;
	timer_load = sec*100000000;
	gornji = (u32)((timer_load & 0xFFFFFFFF00000000LL) >> 32);
	donji = (u32)(timer_load & 0xFFFFFFFFLL);
...
ssize_t timer_read(struct file *pfile, char __user *buffer, size_t length, loff_t *offset) 
{
	...
	u32 data0 = 0, data1 = 0;
	u64 sec = 0;
	unsigned int dani = 0, sati = 0, minuti = 0, sekundi = 0;
	char buff[BUFF_SIZE];
	if (endRead){
		endRead = 0;
		return 0;
	...
	len = scnprintf(buff,BUFF_SIZE,"%u : %u : %u : %u",dani,sati,minuti,sekundi);
